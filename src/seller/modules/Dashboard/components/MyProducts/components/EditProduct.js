...
import { updateProduct, currencyList, deleteProductImage, getAttribute,hsnList,cleaerErrorUdate } from "../sellerAddProductAction";
...
class EditProduct extends Component {
  ...
  validate = () => {
    const errors = {}
      ...
      if(this.state.lowStockThreshold==="")
      errors.lowStockThreshold = `low Stock threshold required`;
        else if(parseInt(this.state.lowStockThreshold)>=parseInt(this.state.stock))
        errors.lowStockThreshold = `low Stock should be less then ${this.state.stock}`;
      ...
      if(this.state.images.length===0 && this.state.imgPreviewUrls.length===0)
   ...
   componentDidMount =async () => {
    ...
    if(productDetail){
      ...
      productDetail.attributes.map((item,index)=>{
        if(item.original.multiValue)
        {
         let  result_value=item.value.split(" ")
         result.push({key:item.key,value:result_value})
         result_value.map((items,index)=>{
           this.setState({[index+item.original.code]:items,ValueCount:this.state.ValueCount+1})
         })
        }
        else{
        result.push({key:item.key,value:item.value})
        this.setState({[item.original.code]:item.value[0]})
        }
      })
    this.setState({
      productId:this.props.match.params.slug,
      type:productDetail.type,
      categoryId: productDetail.category.key,
      brandId:productDetail.brand.key,
      count:1,
      name:productDetail.name,
      listingStatus:productDetail.listingStatus,
      ...
      zipCode: "",
      address: "",
      imgPreviewUrls:productDetail.files,
      originalPrice:productDetail.originalPrice,
      sellingPrice:productDetail.sellingPrice,
      hsn:productDetail.hsn,
      lowStockThreshold:productDetail.lowStockThreshold,
      taxCode: '',
      fulfillmentBy:productDetail.fulfillmentBy,
      stock:productDetail.stock,
      setIsOpen: '',
      procurementSla:productDetail.procurementSla,
      localShippingCost:productDetail.localShippingCost,
      zonalShippingCost:productDetail.zonalShippingCost,
      internationalShippingCost:productDetail.internationalShippingCost,
      packageWeight: productDetail.packageWeight,
      packageLength: productDetail.packageLength,
      packageBreadth:productDetail.packageBreadth,
      packageHeight:productDetail.packageHeight,
      domesticWarranty:productDetail.domesticWarranty,
      internationalWarranty:productDetail.internationalWarranty,
      warrantySummary:productDetail.warrantySummary,
      warrantyServiceType:productDetail.warrantyServiceType==="On Site"?"on-site":"walk-in",
      coveredInWarranty:productDetail.coveredInWarranty,
      notCoveredInWarranty:productDetail.notCoveredInWarranty,
      idealFor:productDetail.idealFor,
      trailer:productDetail.trailer,
      maxQuantityPerOrder:productDetail.maxQuantityPerOrder,
    ...
    },this.props.getAttribute(productDetail.category.key),this.loadData())
  }
  ...
  onSubmitHandler = e => {
    ...
    let files = "files[]";
    this.setState({onSubmitactive:true})
    this.props.cleaerErrorUdate();
    ...
    files: this.state.images.length?this.state.images:null
    ...
    this.props.updateProduct(fd,parseInt(this.state.productId));
    console.log("data submission",productData);
    this.setState({variantButton:false})
  };
  ...
  loadData = (e) => {
    ...
    this.setState({errors:errors || {},disableButton:true});
  }
  saveData = (e) =>{
    if(e){
      e.preventDefault();
   ...
   let {name,listingStatus,originalPrice,sellingPrice,fulfillmentBy,procurementSla,packageWeight,packageLength,packageBreadth,packageHeight,hsn,stock,lowStockThreshold ,sku,attributes,styleCode,images,maxQuantityPerOrder}=this.state;
    if(name && listingStatus && attributes && fulfillmentBy && procurementSla && packageWeight && packageLength && packageBreadth && packageHeight && stock  && styleCode && originalPrice && sellingPrice && sku && hsn && images && lowStockThreshold && maxQuantityPerOrder)
  ...
  createVariant = () => {
    ...
    componentDidUpdate(prevProps, prevState) {
      if(prevProps.updateProductError!==this.props.updateProductError){
        this.setState({disableButton: false});
      }
    }
  ...
  render() {
    ...
    const { updateProductError, AttributesDetails } = this.props;
    const {errors} =this.state;
    console.log("current props edit product",this.props.editProduct);
    return (
        ...
        <div style={{ margin: '10px 15px', paddingTop: '8px', border: '1px solid #00000030', borderRadius: '5px' }}>
            ...
            <div className="row spacing">
             ...
             style={customStyles2
                ...
                {this.state.imgPreviewUrls.map((imagePreviewUrl, i) => {
                  return (
                    <div key={`imgPreviewUrls${i}`} style={{ display: "flex", justifyContent: "center", flexWrap: "wrap", alignItems: "center" }}>
                      <div style={{ color: "red", cursor: "pointer" }} onClick={() => this.imageRemoveHandler(imagePreviewUrl['key'],i)} key={i} >
              ...
              {errors.psCount>0?<span className="" style={{color:"#721c24",background:"#f8d7da",marginLeft:"5px",padding:"2px"}}>{errors.psCount} errors</span>:null}
                  ...
                  <button id="submitButton" type="submit" className="btn-block upaddattribtn" disabled={this.state.disableButton}>Update</button>
                  <br/>
                  {updateProductError && Object.keys(updateProductError).length ? (
                  <p className={updateProductError.status===201?"text-success":"text-danger"}>{updateProductError.res}</p>
                ):this.state.onSubmitactive?<p id="savingProgress" style={{color:"blue"}}>saving in progress....<span></span></p>:""}
              ...
              WARRANTY DETAILS
                <div className="row subrow">
                  ...
                  <div className="col-auto colwidth50">
                    ...
                    <select className="custom-select mr-sm-2" name="warrantyServiceType" value={this.state.warrantyServiceType} onChange={this.onChangeHandler} >
            ...
            <div className="w-100">
              ...
              <div className="row subrow" stylw={{padding:"20px"}}>
                  ...
                  <div className="col-auto colwidth25">
                    ...
                    <div className="col-auto colwidth50"> 
                    <input type="text" name="styleCode" className="form-control" onChange={this.onChangeHandler} value={this.state.styleCode} disabled/>
                    ...
                    return (
                      ...
                      <div className="col-auto colwidth25">
                        ...
                        {res.predefined === true ?
                            ...
                            {res.multiValue === true ?
                              <>
                               {(() => {
                                          ...
                                          <select
                                           className="custom-select mr-sm-2"
                                            ...
                                            {res.values.map((value, index) => {
                                              return (
                                                  <option key={res.code+index} value={value} data-tag={res.key} data-tag2={i}>{value}</option>
                                              )
                                            })}
                                          </select>
                                        )
                                     ...
                                     <button style={{ padding: "10px" }} onClick={this.addMoreattr} disabled={true}>+</button>
                                {/* <Modal
                          ...
                          :
                          <input type="text" name={res.code} className="form-control" value={this.state[res.code]} onChange={(e)=>this.AttributeAddValue(e,res.key,res.useToCreateVariants)}  readOnly={true}/>
                        }
                        <br />
                      </div>
                 ...
                 {errors.attributes && <div className="alert alert-danger">{errors.attributes}</div>}
              </div>
              <Link className="btn btn-primary" onClick={this.saveData}>Save & close</Link>
              <Link className="btn btn-primary" onClick={this.closeModal} style={{marginLeft:"10px"}}>close</Link>
            </div>
              ...
              <div className="col-auto colwidth100">
                ...
                <div className="row subrow">
                  <div className="col-auto colwidth50">
                    ...
                    <input type="text" name="name" value={this.state.name} className="form-control" onChange={this.onChangeHandler} disabled/>
                ...
                <div className="row subrow">
                  ...
                  <div className="col-auto colwidth25">
                    ...
                    <input type="text" name="sku" className="form-control" onChange={this.onChangeHandler} value={this.state.sku} disabled/>
          ...
          {/* <div className="variantContainer">
            ...
            <div className="createVarient">
              <button className="btn btn-primary " onClick={this.createVariant} disabled={this.state.variantButton}>Create Varient</button>
            </div>
            </div>
          </div> */}
...
const mapStateToProps = state => {
  console.log("redux state",state.sellerAddProduct.editproduct);
  return {
    productDetail:state.sellerAddProduct.editproduct,
    updateProductError: state.sellerAddProduct.updateProductError,
...
const mapDispatchToProp = dispatch => {
  return {
    ...
    cleaerErrorUdate:() => dispatch(cleaerErrorUdate()),
