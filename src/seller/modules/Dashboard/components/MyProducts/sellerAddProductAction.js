...
export const currencyList = () => {
  return (dispatch, getState) => {
    axios.get(`${baseURL}/seller/currencies`).then(res => {
      dispatch({
        type: "CURRENCIES",
...
export const categoryList = () => {
  return (dispatch, getState) => {
    axios.get(`${baseURL}/seller/categories`).then(res => {
...
export const countryList = () => {
  return (dispatch, getState) => {
    axios.get(`${baseURL}/seller/countries`).then(res => {
...
export const stateList = (countryId) => {
  return (dispatch, getState) => {
    axios.get(`${baseURL}/seller/countries/${countryId}/states`).then(res => {
...
export const cityList = (stateId) => {
  return (dispatch, getState) => {
    axios.get(`${baseURL}/seller/countries/states/${stateId}/cities`).then(res => {
      dispatch({
        type: "CITY",
        payload: res.data.data
      });
...
export const hsnList = () => {
  return (dispatch) => {
    axios.get(`${baseURL}/seller/hsn`).then(res => {
...
export const attributeList = (categoryId) => {
  return (dispatch, getState) => {
    axios.get(`${baseURL}/seller/categories/${categoryId}/attributes`).then(res => {
      dispatch({
        type: "ATTRIBUTES",
        payload: res.data.data
      });
...
export const generateProductToken = () => {
  return (dispatch) => {
    ...
    axios.get(`${baseURL}/seller/products/token/create`,config)
    .then(res =>{
...
export const addProduct = productData => {
  ...
  return (dispatch, getState) => {
    ...
    axios
      .post(`${baseURL}/seller/products`, productData, config)
      .then(res => {
        let resmessage={
...
export const UploadBulkProduct = (data) =>{
  ...
  return dispatch=>{
    ...
    axios.post(`${baseURL}/seller/bulk`,data,config)
    .then(res=>{
      //console.clear();
...
export const getAllProducts = (current,perPage,status="active") => {
  return dispatch => {
    ...
    axios
      .get(`${baseURL}/seller/products/?status=${status}`,config)
      .then(res => {
...
export const getProduct = slug => {
  return dispatch => {
    ...
    console.log(`${baseURL}/seller/products/${slug}`);
    axios
      .get(`${baseURL}/seller/products/${slug}`, config)
      .then(res => {
        console.clear();
...
export const updateProductStatus = (id,status) => {
  return dispatch => {
    ...
    axios
    .post(`${baseURL}/seller/products/change-status/${id}`,{},config)
    .then(res =>{
      if (res.data.status === 200) {
        dispatch({
          type: "AFTER_DELETE_PRODUCTS",
...
export const editProduct = (slug,history) => {
  return dispatch => {
    ...
    axios
      .get(`${baseURL}/seller/products/edit/${slug}`, config)
      .then(res => {
...
export const updateProduct = (productData,slug,updateStatus) => {
  ...
  return (dispatch, getState) => {
    ...
    axios
      .post(`${baseURL}/seller/products/${slug}`, productData, config)
      .then(res => {
...
export const deleteProductImage = (id) => {
  ...
  //  let request = images.map((id) =>axios.delete(`${baseURL}/seller/products/images/${id}`,config) )
    
   axios.delete(`${baseURL}/seller/products/images/${id}`,config)
...
export const GetBrandNames =(name,category)=>{
  return dispatch=>{
    ...
    let token = localStorage.getItem('token')
    console.log(token)
    axios.get(`${baseURL}/seller/brands`,{ headers: { "Authorization": `Bearer ${token}` },params:{name,category} })
      .then(res=>{
...
export const BrandApproval = (data) =>{
  ...
  return dispatch=>{
    ...
    axios.post(`${baseURL}/seller/brands/approval`,data,config)
    .then(res=>{
      //console.clear();
...
export const getAttribute =(categoryId)=>{
  return dispatch=>{
    let token =localStorage.getItem('token')
    axios.get(`${baseURL}/seller/categories/${categoryId}/attributes`,{ headers: { "Authorization": `Bearer ${token}` } })
    .then(res=>{
...
export const getBrandLists =()=>{
  return dispatch=>{
    let token =localStorage.getItem('token')
    axios.get(`${baseURL}/seller/brands/all`,{ headers: { "Authorization": `Bearer ${token}` } })
    .then(res=>{
      console.log("res data for attribute",res);
      dispatch({
        type: "brandList",
...
export const deleteAttributes = (Attribute) => {
  return dispatch => {
   ...
   // let request = Attribute.map((id) =>axios.delete(`${baseURL}/seller/products/attributes/${Attribute}`,config) )
    
   axios.delete(`${baseURL}/seller/products/attributes/${Attribute}`,config)
 // Promise.all(request)
  .then(res =>{console.log("deleted succesfully")})
      .catch(err => console.log("error in delete ",err));
  };
...
export const FilterBySearch = (currentPage,perPage,query,status) => {
  return dispatch => {
    ...
    axios.get(`${baseURL}/seller/products`, config)
    .then(res => {
