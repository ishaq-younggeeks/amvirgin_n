...
export const myOrderList = (activeState,current,perPage) => {
  ...
  axios.get(`${baseURL}/seller/orders?status=${activeState}`, config)
  .then(res => {
  ...
  export const changeOrderStatusInBulk = (orderId,status) => {
    return dispatch => {
      ...
      let config = {
       ...
       axios.put(`${baseURL}/seller/status/batch-update`,data, config)
       .then(res => {
        console.log("order status",res)
        if (res.data.status === 200) {
          dispatch({
...
export const FilterBySearch = (currentPage,perPage,query,status) => {
  return dispatch => {
    ...
    axios.get(`${baseURL}/seller/orders`, config)
    .then(res => {
...
export const  ViewOrderItem = (id) => {
  return dispatch => {
    let token = localStorage.getItem("token");
    let config = { headers: { Authorization: "Bearer " + token} };
    axios.get(`${baseURL}/seller/orders/${id}`, config)
    .then(res => {
      console.log(res)
      if (res.data.status === 200) {
        dispatch({
