    ...
    .then((res) => {
      ...
      if (res.data.status === 200) {
        ...
        payload.map((item, index) => {
          ...
          doc.text(
            `${item.seller.rbaSecondLine} ${item.seller.rbaCity.name} ${item.seller.rbaState.name},`,
            20,
            50
          );
    ...
    .then((res) => {
      ...
      if (res.data.status === 200) {
        ...
        payload.map((item, index) => {
          ...
          doc.setFontSize(8);
          doc.text(
            "TO BE FILLED BY EKART LOGISTICS EXECUTIVE",
            60,
            finalY + 10
          );
          ...
          doc.save(`manifest_${item.order.key}.pdf`);
        });
        console.log("working download label", res);
      }
    })
